...
class Mk2shellMakeShell extends Command{
    ...
    */
    public function __construct($argv){
        ...
        $this->green("Create a new Shell file.");
        $this->text("");
        ...
        if(!empty($argv[0])){
            ...
            $buff="";
            for(;;){
                $buff=$this->input("\t- Enter the name of the Shell to create.");
                if($buff){
                    break;
                }
                $this->red("\t  [ERROR] The Shell name has not been entered.");
            }
            $input["name"]=$buff;
        }

        $input["extends"]=$this->input("\t- If there is an inheritance source Shell name, enter it.");
        ...
        $this->text("");
        $juge=strtolower($this->input("\t- Create a Shell file based on the entered information. Is it OK?[Y/n]"));
        
        if($juge=="n"){
            $this->text("");
            $this->text("");
            $this->text("Shell creation has been canceled,");
            return;
        }
    ...
    private function _make($data){
        ...
        $str.=" * ============================================\n";
        $str.=" */ \n";
        $str.="namespace App\Shell;\n";
        $str.="\n";
        if(!$data["extends"]){
            $str.="use Mk2\Libraries\Shell;\n";
            $data["extends"]="";
            $str.="\n";
        }
        $str.="class ".ucfirst($data["name"])."Shell extends ".ucfirst($data["extends"])."Shell\n";
        $str.="{\n";
        ...
        $fileName=MK2_ROOT."/".MK2_DEFNS_SHELL."/".ucfirst($data["name"])."Shell.php";
        $fileName=str_replace("\\","/",$fileName);

        if(file_exists($fileName)){
            $juge=strtolower($this->input("\tThe same Shell already exists, do you want to overwrite it as it is?[y/n]"));
            if($juge!="y"){ $juge="n"; }
